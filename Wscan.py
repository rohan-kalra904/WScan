...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"WScan received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/wscan_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/wscan_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/wscan_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/wscan_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/wscan_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o wscan_temp_wapiti"],
...
os.system('rm /tmp/wscan* > /dev/null 2>&1') # Clearing previous scan files
os.system('clear')
...
while (rs_avail_tools < len(tools_precheck)):
    ...
    try:
        ...
        print("\t"+bcolors.BG_ERR_TXT+"WScan was terminated abruptly..."+bcolors.ENDC)
        sys.exit(1)
...
if len(unavail_tools_names) == 0:
    print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by WScan."+bcolors.ENDC)
else:
    print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. WScan will still perform the rest of the tests. Install these tools to fully utilize the functionality of WScan."+bcolors.ENDC)
...
while(tool < len(tool_names)):
    ...
    temp_file = "/tmp/wscan_temp_"+tool_names[tool][arg1]
    ...
    if runTest == 1:
            ...
            print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit WScan.\n" + bcolors.ENDC)
            rs_skipped_checks = rs_skipped_checks + 1
...
if len(rs_vul_list)==0:
    ...
    with open(vulreport, "a") as report:
        while(rs_vul < len(rs_vul_list)):
            ...
            report.write("\n------------------------\n\n")
            temp_report_name = "/tmp/wscan_temp_"+vuln_info[arg1]
        ...
        print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory WScan resides.")
...
for file_index, file_name in enumerate(tool_names):
    with open(debuglog, "a") as report:
        try:
            with open("/tmp/wscan_temp_"+file_name[arg1], 'r') as temp_report:
                    data = temp_report.read()
...
os.system('rm /tmp/wscan_te* > /dev/null 2>&1') # Clearing previous scan files
